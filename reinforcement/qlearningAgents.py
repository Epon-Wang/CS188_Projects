...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        if not actions:
            return 0.0
        return max(
            self.getQValue(state, a) for a in actions
        )
    ...
    def update(self, state, action, nextState, reward):
        ...
        td_error = self.alpha * (self.getQValue(state, action) - td_target)
        self.q_table[(state, action)] -= td_error
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        features = self.featExtractor.getFeatures(state, action)
        for f in features:
            self.weights[f] -= td_error * features[f]
